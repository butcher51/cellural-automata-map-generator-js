...
import { generateMap, getCellColor, countNeighborFriends, calculateAllFriendCounts, applyCaveRules, generateOrganicMap, deepCopyMap, pixelToGridCoordinate, toggleCellValue, setCellValue, applyOrganicIterations, getCellsInBrushArea, getCellColorWithDrawingState, updateCamera, clampCamera, alignToBlockGrid, expandBlockMapToFullMap, contractFullMapToBlockMap, setBlockValue, applyOrganicIterationsBlockAligned, generateOrganicMapBlockAligned } from './map-utils.js';
...
describe('alignToBlockGrid', () => {
    it('should return (0,0) for coordinates (0,0) with blockDim 2', () => {
        const result = alignToBlockGrid(0, 0, 2);
        expect(result).toEqual({ x: 0, y: 0 });
    });

    it('should return (0,0) for coordinates (1,1) with blockDim 2', () => {
        const result = alignToBlockGrid(1, 1, 2);
        expect(result).toEqual({ x: 0, y: 0 });
    });

    it('should return (2,4) for coordinates (3,5) with blockDim 2', () => {
        const result = alignToBlockGrid(3, 5, 2);
        expect(result).toEqual({ x: 2, y: 4 });
    });

    it('should return (2,2) for coordinates (2,2) with blockDim 2', () => {
        const result = alignToBlockGrid(2, 2, 2);
        expect(result).toEqual({ x: 2, y: 2 });
    });

    it('should return (4,6) for coordinates (5,7) with blockDim 2', () => {
        const result = alignToBlockGrid(5, 7, 2);
        expect(result).toEqual({ x: 4, y: 6 });
    });

    it('should work with blockDim 4', () => {
        expect(alignToBlockGrid(0, 0, 4)).toEqual({ x: 0, y: 0 });
        expect(alignToBlockGrid(3, 3, 4)).toEqual({ x: 0, y: 0 });
        expect(alignToBlockGrid(4, 4, 4)).toEqual({ x: 4, y: 4 });
        expect(alignToBlockGrid(7, 5, 4)).toEqual({ x: 4, y: 4 });
    });

    it('should handle large coordinates', () => {
        const result = alignToBlockGrid(99, 98, 2);
        expect(result).toEqual({ x: 98, y: 98 });
    });

    it('should handle coordinates at block boundaries', () => {
        // At exact boundary, should return that boundary
        expect(alignToBlockGrid(10, 10, 2)).toEqual({ x: 10, y: 10 });
        expect(alignToBlockGrid(11, 11, 2)).toEqual({ x: 10, y: 10 });
    });
});

describe('expandBlockMapToFullMap', () => {
    it('should double the dimensions of a 2x2 block map with blockDim 2', () => {
        const blockMap = [
            [{ value: 1 }, { value: 0 }],
            [{ value: 0 }, { value: 1 }]
        ];
        const result = expandBlockMapToFullMap(blockMap, 2);

        expect(result.length).toBe(4);
        expect(result[0].length).toBe(4);
    });

    it('should replicate each block cell to 2x2 area with blockDim 2', () => {
        const blockMap = [
            [{ value: 1 }, { value: 0 }],
            [{ value: 0 }, { value: 1 }]
        ];
        ...
        // Top-left 2x2 should all be value 1
        expect(result[0][0].value).toBe(1);
        expect(result[0][1].value).toBe(1);
        expect(result[1][0].value).toBe(1);
        expect(result[1][1].value).toBe(1);

        // Top-right 2x2 should all be value 0
        expect(result[0][2].value).toBe(0);
        expect(result[0][3].value).toBe(0);
        expect(result[1][2].value).toBe(0);
        expect(result[1][3].value).toBe(0);

        // Bottom-left 2x2 should all be value 0
        expect(result[2][0].value).toBe(0);
        expect(result[2][1].value).toBe(0);
        expect(result[3][0].value).toBe(0);
        expect(result[3][1].value).toBe(0);

        // Bottom-right 2x2 should all be value 1
    ...
    it('should handle single cell block map', () => {
        const blockMap = [[{ value: 1 }]];
        const result = expandBlockMapToFullMap(blockMap, 2);
    ...
    it('should preserve isBeingDrawn property', () => {
        const blockMap = [[{ value: 1, isBeingDrawn: true }]];
        const result = expandBlockMapToFullMap(blockMap, 2);

        expect(result[0][0].isBeingDrawn).toBe(true);
        expect(result[0][1].isBeingDrawn).toBe(true);
        expect(result[1][0].isBeingDrawn).toBe(true);
        expect(result[1][1].isBeingDrawn).toBe(true);
    });
    ...
    it('should create independent cell objects (not shared references)', () => {
        const blockMap = [[{ value: 1 }]];
        const result = expandBlockMapToFullMap(blockMap, 2);
    ...
    it('should correctly expand a 3x3 block map to 6x6', () => {
        const blockMap = [
            [{ value: 1 }, { value: 0 }, { value: 1 }],
            [{ value: 0 }, { value: 1 }, { value: 0 }],
            [{ value: 1 }, { value: 0 }, { value: 1 }]
        ...
        // Check center block (1,1) becomes (2,2), (2,3), (3,2), (3,3)
        expect(result[2][2].value).toBe(1);
        expect(result[2][3].value).toBe(1);
        expect(result[3][2].value).toBe(1);
        expect(result[3][3].value).toBe(1);
...
describe('contractFullMapToBlockMap', () => {
    it('should halve the dimensions of a 4x4 full map with blockDim 2', () => {
        const fullMap = [
        ...
        const result = contractFullMapToBlockMap(fullMap, 2);

        expect(result.length).toBe(2);
        expect(result[0].length).toBe(2);
    });

    it('should sample the top-left cell of each block', () => {
        const fullMap = [
        ...
        // Top-left block samples (0,0)
        expect(result[0][0].value).toBe(1);
        // Top-right block samples (0,2)
        expect(result[0][1].value).toBe(0);
        // Bottom-left block samples (2,0)
        expect(result[1][0].value).toBe(0);
        // Bottom-right block samples (2,2)
        expect(result[1][1].value).toBe(1);
    });

    it('should handle single block (2x2 full map)', () => {
        const fullMap = [
            [{ value: 1 }, { value: 1 }],
            [{ value: 1 }, { value: 1 }]
        ];
    ...
    it('should preserve cell properties', () => {
        const fullMap = [
            [{ value: 1, friendCount: 5, isBeingDrawn: true }, { value: 1 }],
            [{ value: 1 }, { value: 1 }]
        ];
        const result = contractFullMapToBlockMap(fullMap, 2);
    ...
    it('should create independent cell objects', () => {
        ...
        expect(fullMap[0][0].value).toBe(1);
    });

    it('should correctly contract a 6x6 full map to 3x3 block map', () => {
        // Create a 6x6 map with distinct values at each block's top-left
        const fullMap = [];
        for (let y = 0; y < 6; y++) {
            fullMap[y] = [];
            for (let x = 0; x < 6; x++) {
                ...
                fullMap[y][x] = { value: (blockY + blockX) % 2 };
            }
        }
        ...
        // Verify values match the checkerboard pattern
        expect(result[0][0].value).toBe(0);
        expect(result[0][1].value).toBe(1);
        expect(result[0][2].value).toBe(0);
        expect(result[1][0].value).toBe(1);
    ...
    it('should be inverse of expandBlockMapToFullMap', () => {
        const original = [
            [{ value: 1 }, { value: 0 }],
            [{ value: 0 }, { value: 1 }]
        ];
        const expanded = expandBlockMapToFullMap(original, 2);
        const contracted = contractFullMapToBlockMap(expanded, 2);

        expect(contracted.length).toBe(original.length);
        expect(contracted[0].length).toBe(original[0].length);
        expect(contracted[0][0].value).toBe(original[0][0].value);
        expect(contracted[0][1].value).toBe(original[0][1].value);
        expect(contracted[1][0].value).toBe(original[1][0].value);
        expect(contracted[1][1].value).toBe(original[1][1].value);
    });
});

describe('setBlockValue', () => {
    it('should set all 4 cells in a 2x2 block to the same value', () => {
        const map = [
        ...
        const result = setBlockValue(map, 0, 0, 1, 2);
        ...
        // Other blocks should remain 0
        expect(result[0][2].value).toBe(0);
        expect(result[2][0].value).toBe(0);
    });

    it('should set block at non-zero position', () => {
        const map = [
            [{ value: 0 }, { value: 0 }, { value: 0 }, { value: 0 }],
            [{ value: 0 }, { value: 0 }, { value: 0 }, { value: 0 }],
            [{ value: 0 }, { value: 0 }, { value: 0 }, { value: 0 }],
        ...
        // Block (1,1) with blockDim=2 covers cells (2,2), (2,3), (3,2), (3,3)
        const result = setBlockValue(map, 1, 1, 1, 2);

        // Bottom-right 2x2 block should all be 1
        expect(result[2][2].value).toBe(1);
        expect(result[2][3].value).toBe(1);
        expect(result[3][2].value).toBe(1);
        expect(result[3][3].value).toBe(1);

        // Top-left block should remain 0
        expect(result[0][0].value).toBe(0);
    });

    it('should not modify original map (immutability)', () => {
        const original = [
            [{ value: 0 }, { value: 0 }],
            [{ value: 0 }, { value: 0 }]
        ];
        const result = setBlockValue(original, 0, 0, 1, 2);

        expect(original[0][0].value).toBe(0);
        expect(result[0][0].value).toBe(1);
    });

    it('should set isBeingDrawn to true for all cells in block', () => {
        const map = [
            [{ value: 0, isBeingDrawn: false }, { value: 0, isBeingDrawn: false }],
            [{ value: 0, isBeingDrawn: false }, { value: 0, isBeingDrawn: false }]
        ];
        const result = setBlockValue(map, 0, 0, 1, 2);
    ...
    it('should handle block coordinates (not cell coordinates)', () => {
        // blockX=1, blockY=1 with blockDim=2 should set cells (2,2), (2,3), (3,2), (3,3)
        const map = [
            [{ value: 0 }, { value: 0 }, { value: 0 }, { value: 0 }],
            [{ value: 0 }, { value: 0 }, { value: 0 }, { value: 0 }],
            [{ value: 0 }, { value: 0 }, { value: 0 }, { value: 0 }],
        ...
        // Block (1,1) spans full cells (2-3, 2-3)
        expect(result[2][2].value).toBe(1);
        expect(result[2][3].value).toBe(1);
        expect(result[3][2].value).toBe(1);
        expect(result[3][3].value).toBe(1);
    ...
    it('should return original map for out of bounds block coordinates', () => {
        const original = [
            [{ value: 0 }, { value: 0 }],
            [{ value: 0 }, { value: 0 }]
        ];
        const result = setBlockValue(original, 5, 5, 1, 2);
        expect(result).toEqual(original);
    });

    it('should return original map for negative block coordinates', () => {
        const original = [
            [{ value: 0 }, { value: 0 }],
            [{ value: 0 }, { value: 0 }]
        ];
        const result = setBlockValue(original, -1, 0, 1, 2);
        expect(result).toEqual(original);
    });

    it('should return original map for invalid value', () => {
        const original = [
            [{ value: 0 }, { value: 0 }],
            [{ value: 0 }, { value: 0 }]
        ];
        const result = setBlockValue(original, 0, 0, 5, 2);
        expect(result).toEqual(original);
    });
    ...
    it('should preserve other cell properties', () => {
        const map = [
            [{ value: 0, friendCount: 3, customProp: 'test' }, { value: 0 }],
            [{ value: 0 }, { value: 0 }]
        ];
        const result = setBlockValue(map, 0, 0, 1, 2);
...
describe('applyOrganicIterationsBlockAligned', () => {
    it('should return map with correct dimensions', () => {
        const map = generateMap(10);
        const result = applyOrganicIterationsBlockAligned(map, 1, 2);

        expect(result.length).toBe(10);
        expect(result[0].length).toBe(10);
    });

    it('should have friendCount calculated on all cells', () => {
        const map = generateMap(10);
        const result = applyOrganicIterationsBlockAligned(map, 1, 2);
    ...
    it('should clear all isBeingDrawn flags', () => {
        // Create map with some isBeingDrawn flags set
        const map = [];
        for (let y = 0; y < 4; y++) {
            map[y] = [];
            for (let x = 0; x < 4; x++) {
                map[y][x] = { value: Math.random() > 0.5 ? 1 : 0, isBeingDrawn: true };
            }
        }
    ...
    it('should not modify original map (immutability)', () => {
        ...
        const result = applyOrganicIterationsBlockAligned(original, 1, 2);

        // Original should not have friendCount added
        expect(original[0][0]).not.toHaveProperty('friendCount');
        expect(result[0][0]).toHaveProperty('friendCount');
    });

    it('should handle 0 iterations', () => {
        const map = generateMap(4);
        const result = applyOrganicIterationsBlockAligned(map, 0, 2);
...
describe('generateOrganicMapBlockAligned', () => {
    it('should return map with correct dimensions', () => {
        const map = generateOrganicMapBlockAligned(10, 1, 2);

        expect(map.length).toBe(10);
        expect(map[0].length).toBe(10);
    });

    it('should have friendCount on all cells', () => {
        const map = generateOrganicMapBlockAligned(10, 1, 2);
    ...
    it('should have isBeingDrawn false on all cells', () => {
        const map = generateOrganicMapBlockAligned(10, 1, 2);
    ...
    it('should contain only 0 and 1 values', () => {
        const map = generateOrganicMapBlockAligned(10, 2, 2);
    ...
    it('should work with size 0', () => {
        const map = generateOrganicMapBlockAligned(0, 1, 2);
        expect(map.length).toBe(0);
    });

    it('should handle 0 iterations', () => {
        const map = generateOrganicMapBlockAligned(10, 0, 2);
...
describe('block alignment validation', () => {
    it('should maintain homogeneous 2x2 blocks after generateOrganicMapBlockAligned', () => {
        const map = generateOrganicMapBlockAligned(10, 3, 2);

        // Check that each 2x2 block has the same value in all 4 cells
        for (let blockY = 0; blockY < 5; blockY++) {
            for (let blockX = 0; blockX < 5; blockX++) {
                const startY = blockY * 2;
                const startX = blockX * 2;
                const blockValue = map[startY][startX].value;

                expect(map[startY][startX + 1].value).toBe(blockValue);
                expect(map[startY + 1][startX].value).toBe(blockValue);
                expect(map[startY + 1][startX + 1].value).toBe(blockValue);
            }
        }
    });

    it('should maintain homogeneous blocks after applyOrganicIterationsBlockAligned', () => {
        // Create a block-aligned map
        const initialMap = generateOrganicMapBlockAligned(8, 0, 2);

        // Apply iterations
        const result = applyOrganicIterationsBlockAligned(initialMap, 2, 2);
        ...
        for (let blockY = 0; blockY < 4; blockY++) {
            for (let blockX = 0; blockX < 4; blockX++) {
                const startY = blockY * 2;
                const startX = blockX * 2;
                const blockValue = result[startY][startX].value;

                expect(result[startY][startX + 1].value).toBe(blockValue);
                expect(result[startY + 1][startX].value).toBe(blockValue);
                expect(result[startY + 1][startX + 1].value).toBe(blockValue);
            }
        }
    });

    it('should generate different blocks (randomness check)', () => {
        const map = generateOrganicMapBlockAligned(20, 3, 2);

        // With a 20x20 map (100 blocks), we should have both 0s and 1s
        const flatMap = map.flat();
        const hasZeros = flatMap.some(cell => cell.value === 0);
        const hasOnes = flatMap.some(cell => cell.value === 1);
