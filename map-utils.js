...
export function countNeighborFriends(map, x, y) {
    ...
    if (x < 0 || !map[y] || x >= map[y].length) return 0;
...
// Snap coordinates to block grid boundary (returns top-left of block)
// blockDim: the size of blocks (e.g., 2 for 2x2 blocks)
export function alignToBlockGrid(x, y, blockDim) {
    return {
        x: Math.floor(x / blockDim) * blockDim,
        y: Math.floor(y / blockDim) * blockDim
    };
}

// Expand a block map to full resolution
// Each block cell becomes a blockDim x blockDim group of identical cells
// blockMap: smaller map at block resolution
// blockDim: size of blocks (e.g., 2 means each block cell becomes 2x2)
// Returns: full resolution map
export function expandBlockMapToFullMap(blockMap, blockDim) {
    if (!blockMap || blockMap.length === 0) return [];

    const blockHeight = blockMap.length;
    const blockWidth = blockMap[0].length;
    const fullHeight = blockHeight * blockDim;
    const fullWidth = blockWidth * blockDim;

    const fullMap = [];

    for (let fullY = 0; fullY < fullHeight; fullY++) {
        fullMap[fullY] = [];
        for (let fullX = 0; fullX < fullWidth; fullX++) {
            // Calculate which block this cell belongs to
            const blockY = Math.floor(fullY / blockDim);
            const blockX = Math.floor(fullX / blockDim);

            // Copy cell properties from block cell
            const blockCell = blockMap[blockY][blockX];
            fullMap[fullY][fullX] = { ...blockCell };
        }
    }
...
// Contract a full resolution map to block resolution
// Samples the top-left cell of each blockDim x blockDim block
// fullMap: full resolution map
// blockDim: size of blocks (e.g., 2 means sample every 2x2 block)
// Returns: block resolution map
export function contractFullMapToBlockMap(fullMap, blockDim) {
    if (!fullMap || fullMap.length === 0) return [];

    const fullHeight = fullMap.length;
    const fullWidth = fullMap[0].length;
    const blockHeight = Math.floor(fullHeight / blockDim);
    const blockWidth = Math.floor(fullWidth / blockDim);
    ...
    for (let blockY = 0; blockY < blockHeight; blockY++) {
        ...
        for (let blockX = 0; blockX < blockWidth; blockX++) {
            // Sample the top-left cell of each block
            const fullY = blockY * blockDim;
            const fullX = blockX * blockDim;

            // Copy cell properties from top-left of block
            const fullCell = fullMap[fullY][fullX];
            blockMap[blockY][blockX] = { ...fullCell };
        }
    }
...
// Set all cells in a block to the specified value
// Takes block coordinates (not cell coordinates)
// blockX, blockY: block position (e.g., (1,1) refers to block starting at cell (2,2) with blockDim=2)
// value: 0 or 1
// blockDim: size of blocks
// Returns: new map (immutable operation)
export function setBlockValue(map, blockX, blockY, value, blockDim) {
    // Validate inputs
    if (!map || map.length === 0) return map;
    if (value !== 0 && value !== 1) return map;
    if (blockX < 0 || blockY < 0) return map;

    const fullHeight = map.length;
    const fullWidth = map[0].length;

    // Calculate cell coordinates for this block
    const startX = blockX * blockDim;
    const startY = blockY * blockDim;
    const endX = startX + blockDim;
    const endY = startY + blockDim;

    // Validate bounds
    if (startX >= fullWidth || startY >= fullHeight) return map;
    if (endX > fullWidth || endY > fullHeight) return map;

    // Deep copy the map
    const newMap = deepCopyMap(map);

    // Set all cells in the block
    for (let y = startY; y < endY; y++) {
        for (let x = startX; x < endX; x++) {
            newMap[y][x].value = value;
            newMap[y][x].isBeingDrawn = true;
...
// Apply N iterations of cellular automaton at block level, maintaining block alignment
// fullMap: full resolution map
// iterations: number of CA iterations to run
// blockDim: size of blocks (e.g., 2 for 2x2 blocks)
// Returns: full resolution map with block-aligned values
export function applyOrganicIterationsBlockAligned(fullMap, iterations, blockDim) {
    if (!fullMap || fullMap.length === 0) return [];

    // Contract to block resolution
    let blockMap = contractFullMapToBlockMap(fullMap, blockDim);

    // Run CA iterations at block level
    for (let i = 0; i < iterations; i++) {
        calculateAllFriendCounts(blockMap);
        applyCaveRules(blockMap);
    }

    // Expand back to full resolution
    let result = expandBlockMapToFullMap(blockMap, blockDim);

    // Calculate final friendCount at full resolution for rendering
    calculateAllFriendCounts(result);

    // Clear all isBeingDrawn flags
    for (let y = 0; y < result.length; y++) {
        for (let x = 0; x < result[y].length; x++) {
            result[y][x].isBeingDrawn = false;
        }
    }
...
// Generate a block-aligned map from scratch
// size: full map size (should be divisible by blockDim)
// iterations: number of CA iterations to run
// blockDim: size of blocks
// Returns: full resolution map with block-aligned values
export function generateOrganicMapBlockAligned(size, iterations, blockDim) {
    if (size === 0) return [];

    // Generate random map at block resolution
    const blockSize = Math.floor(size / blockDim);
    const blockMap = generateMap(blockSize);
