 ...
 * neighbor. Out-of-bounds cells are treated as water, so deep water can extend
 * to map edges. After `gap` iterations, only cells that are at least `gap`
 * tiles away from land remain.
 *
  ...
  for (let iter = 0; iter < gap; iter++) {
    ...
    for (let y = 0; y < height; y++) {
      for (let x = 0; x < width; x++) {
        ...
        // Check if has non-water cardinal neighbor (OOB treated as water)
        if (
          getCell(current, y - 1, x) !== 1 ||
          getCell(current, y + 1, x) !== 1 ||
          getCell(current, y, x - 1) !== 1 ||
          getCell(current, y, x + 1) !== 1
        ) {
          next[y][x] = 0;
        }
